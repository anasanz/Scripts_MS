...
# Time frame STDEC (Turca)

tor <- read.csv("stdec_ds_ready_02_21.csv", sep = ",")
tor[,1] <- "STTUR" # Aunque sea STDEC lo dejo asi para que no me de problemas el script
...
temp_add <- data.frame(matrix(NA, ncol = length(yrs) + 3, nrow = length(transect_add)))
colnames(temp_add) <- c("transect_ID", "year_start", "year_end", yrs)
temp_add$transect_ID <- transect_add
...
for (xxx in 1:length(transect_add)){
  ...
  temp_add[xxx,2:3] <- c(min_year,max_year)
  temp_add[xxx,4:23] <- ifelse(is.na(data_transect[1,-21]), 0, 1)
}
...
# Join al
temp_all <- rbind(temp,temp_add)
temp_all$transect_ID <- as.numeric(temp_all$transect_ID)
temp_all <- temp_all %>% arrange(transect_ID)
...
temp_all_subset <- temp_all[which(temp_all$transect_ID %in% tr_tor$transect_ID), ]
