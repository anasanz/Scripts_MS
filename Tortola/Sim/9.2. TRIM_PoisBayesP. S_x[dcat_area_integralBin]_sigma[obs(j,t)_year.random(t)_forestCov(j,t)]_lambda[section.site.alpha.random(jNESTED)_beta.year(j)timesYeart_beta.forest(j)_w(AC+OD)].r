...
# Model 9.2: 8 + remove random lam.year + MULTIPLY B.YEAR*YEAR(T)
...
# N[jt] ~ Pois(lambda[jt])
# log(lambda[jt]) <- site_sec.alpha.ran[j] + beta*yr[t-1] + forest[j] + W
...
cat("model{
    ...
    lambda[j,1] <- exp(lam.section[sitessections[j], sections[j]] + bYear.lam*year_index[1] + bforest.lam*forestCov[j] + w[j,1]) # year1 is t-1; year_index is t (to index properly the random effect) #### MULTIPLICAR BETA*YEAR (EN VEZ DE YEAR T- 1) POR PROBAR
    w[j,1] <- eps[j,1] / sqrt(1 - rho * rho)
    eps[j,1] ~ dnorm(0, tau)
    ...
    lambda[j,t] <- exp(lam.section[sitessections[j], sections[j]] + bYear.lam*year_index[t] + bforest.lam*forestCov[j] + w[j,t]) #### MULTIPLICAR BETA*YEAR (EN VEZ DE YEAR T- 1) POR PROBAR
    w[j,t] <- rho * w[j,t-1] + eps[j,t]
    eps[j,t] ~ dnorm(0, tau)
...
params <- c( "mu.sig", "sig.sig", "bforest.sig", "sig.obs", "log.sigma.year", # Save also observer effect
             "mu.site", "sig.site", "sig.section", "bYear.lam", "bforest.lam", # Save year effect
             "popindex", "sd", "rho", "lam.tot",'Bp.Obs', 'Bp.N'
)


# MCMC settings
nc <- 3 ; ni <- 200000 ; nb <- 3000 ; nt <- 5

# With jagsUI 
# With jagsUI 
out <- jags(data1, inits, params, "model9.txt", n.chain = nc,
...
setwd("D:/Ana/Model/Tórtola")
save(out, file = "9.RData") # 60000 iter, 4 thining
