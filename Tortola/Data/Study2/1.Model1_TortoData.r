...
var <- data_buff500[ ,c("Site","Section","site_sec", "Hm_mean", "Hm_max", "FCC_mean", "FCC100%", "Forest%", "richness", "ForestMargin", "ForestMargin%")]
var2 <- var[,which(colnames(var) %in% c("site_sec", "Site","Section","Forest%"))]
# Format
forest <- matrix(NA, nrow = nrow(m_subset2), ncol = ncol(m_subset2))
rownames(forest) <- rownames(m_subset2)
colnames(forest) <- colnames(m_subset2)
...
# Manually add data NA
forest[which(rownames(forest) %in% "530_3"), ] <- (3.0630105+2.9421069)/2
forest[which(rownames(forest) %in% "141_6"), ] <- 1 # Very little
forest[which(rownames(forest) %in% "207_1"), ] <- 0 # En google maps seems there is no forest
forest[which(rownames(forest) %in% "207_2"), ] <- 0
forest[which(rownames(forest) %in% "207_3"), ] <- 0

forest <- as.data.frame(forest) 
forest$site <- m_subset2$site # For subset
...
for (xxx in 1:length(transect)){
  ...
  # Forest variable
  forest_transect <- as.matrix(forest[which(forest$site %in% transect[xxx]), which(colnames(forest) %in% colnames(m))])
  
  forest_mean <- mean(forest_transect)
  forest_sd <- sd(forest_transect)
  forest_sc <- (forest_transect - forest_mean) / forest_sd
  ...
  cat("model{
    ...
    sigma[j,1] <- exp(log.sigma.year[year_index[1]] + bforest.sig*forestCov[j,1])
    ...
    sigma[j,t] <- exp(log.sigma.year[year_index[t]] + bforest.sig*forestCov[j,t])
  ...
  inits <- function(){list(mu.lam.year = runif(1), sig.lam.year = runif(1), bYear.lam = runif(1),
                           mu.sig.year = runif(1, log(30), log(50)), sig.sig.year = runif(1), bforest.sig = runif(1),
                           N = Nst)} 
  
  # Params
  params <- c( "mu.lam.year", "sig.lam.year", "bYear.lam", "mu.sig.year", "sig.sig.year", "bforest.sig", 
  ...
  setwd("D:/Otros/Tórtola/Results/Study2/Model1")
  save(out, file = paste("1TortoData", "_", transect[xxx], ".RData"), sep = "")
