...
for (i in 1:length(transect)){
  load(paste("1.1TortoData", "_", transect[i],"_", "5e5iter", ".RData", sep = ""))
...
save(no_converge, file = "1.1TortoData_noconverge.RData") # Save to run at 500000
...
for (i in 1:length(no_converge)){
  load(paste("1.1TortoData", "_", no_converge[i],"_", "5e5iter", ".RData", sep = ""))
...
setwd("S:/Results/Otros/Tortola/Study2/Model1.1")
files <-paste("1.1TortoData", "_", converge,"_", "5e5iter", ".RData", sep = "") # remove the ones that did not converge
...
pdf("trend_transects_model1.1.pdf", width = 9, height = 7)
...
# Roughness tiene efecto en la detectabilidad de 9/38 transectos: 6 efecto negativo, 2 efecto positivo?