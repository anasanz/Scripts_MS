...
# Model 9: 8 + remove random lam.year
...
cat("model{
    ...
    lambda[j,1] <- exp(lam.section[sitessections[j], sections[j]] + bYear.lam*year1[1] + bforest.lam*forestCov[j] + w[j,1]) # year1 is t-1; year_index is t (to index properly the random effect)
    w[j,1] <- eps[j,1] / sqrt(1 - rho * rho)
    eps[j,1] ~ dnorm(0, tau)
    ...
    lambda[j,t] <- exp(lam.section[sitessections[j], sections[j]] + bYear.lam*year1[t] + bforest.lam*forestCov[j] + w[j,t])
    w[j,t] <- rho * w[j,t-1] + eps[j,t]
    eps[j,t] ~ dnorm(0, tau)
