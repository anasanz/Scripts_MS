...
for (xxx in 1:length(transect)){
  ...
  data1 <- list(nyears = nyrs, nsection = nSection, nG=nG, int.w=int.w, strip.width = strip.width, midpt = midpt, db = dist.breaks,
                ...
                year1 = year_number, section = section, year_index = yrs)
  ...
  cat("model{
      ...
      sigma[j,1] <- exp(log.sigma.year[year_index[1]])
      ...
      sigma[j,t] <- exp(log.sigma.year[year_index[t]])
      ...
      }",fill=TRUE, file = "Model0_study2.txt")
  
    # Inits
    Nst <- m_index + 1
    inits <- function(){list(mu.lam.year = runif(1), sig.lam.year = runif(1), bYear.lam = runif(1),
                             mu.sig.year = runif(1, log(30), log(50)), sig.sig.year = runif(1),
                             N = Nst)} 
    
    # Params
    params <- c( "mu.lam.year", "sig.lam.year", "bYear.lam", "mu.sig.year", "sig.sig.year", 
    ...
    # With jagsUI 
    out <- jags(data1, inits, params, "Model0_study2.txt", n.chain = nc,
                n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
    summary <- out$summary
    print(out)
    
    
    setwd("D:/Otros/Tórtola/Results/Study2")
    save(out, file = paste("0TortoData", "_", transect[xxx], ".RData"))
}
