...
m_subset <- m_subset[which(m_subset$site %in% transect_add), ]
...
for (xxx in 1:length(transect)){
  ...
  data_transect <- m_subset[which(m_subset$site %in% transect[xxx]), ]
  ...
  #setwd("~/Scripts_MS/Tortola/Run_Data/Study2")
  ...
  #setwd("~/Scripts_MS/Tortola/Model_results/Models_added_transects")
  save(out, file = paste("1.1TortoData_0221_", transect[xxx], "_9e5iter.RData"), sep = "")
