...
# Seleccionar años del 2002 al 2010
m <- m[,c(1:9)]
# Remove transects only made 2 years
m <- m[-which(rowSums(is.na(m)) > 6), ]
...
# Update dataset "tor" with new sites (deleted sites with less than 3 years sampled)
...
for_id <- unique(factor(forest))[-45]
...
save(out, file = "7tor.RData")

###############################################################################################

setwd("S:/Results/Otros/Tortola")
load("7tor_1.3.RData")
...
setwd("D:/PhD/Otros/Tórtola/Results")
pdf("1.3.Trend.pdf", 7,5 )

par(mfrow = c(1,2), 
    mar = c(5,3,4,1))

plot(-1000, xlim=c(0,8), ylim=c(0,max(uci.exp)+20), main = " ", xlab = " ", ylab = " ", axes = FALSE)
axis(2)
axis(1,at = c(0:8), labels = c("2002", "2003", "2004", "2005", "2006", "2007", "2008", "2009", "2010"))
...
dens_year <- density(outall[ ,which(colnames(outall) == "bYear.lam")]) # Density of iterations for bYear.lam
mode_year <- dens_year$x[dens_year$y == max(dens_year$y)]
mean_year <- mean(outall[ ,which(colnames(outall) == "bYear.lam")])

plot(dens_year, xlim = c(-0.2, 0.2), xlab = "Posterior distribution bYear", ylab = " ", main = " ", yaxs = "i") 

polygon(c(dens_year$x[dens_year$x < 0], 0), c(dens_year$y[dens_year$x < 0],0 ), col="grey") # ?? I still don't know if its right
abline( v = 0, col = "black", lwd = 1)

abline( v = mean_year, col = "red", lwd = 1.5)

mtext(paste("PD =", round(prob_declining,2)), side = 3, line = -1.5, cex = 1, adj = 0.05)

mtext("Population trend 2002-2010", side = 3, line = -3, cex = 1, outer = TRUE)
