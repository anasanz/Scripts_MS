...
#'
#' The \code{dbinomLocal_normalBear} distribution is a NIMBLE custom distribution which can be used to model 
...
#' 
#' The \code{dbinomLocal_normalBear} distribution incorporates three features to increase computation efficiency (see Turek et al., 2021 <doi.org/10.1002/ecs2.3385>  for more details):
#' \enumerate{
...
#' }
#' 
#' The \code{dbinomLocal_normalBear} distribution requires x- and y- detector coordinates (\emph{trapCoords}) and activity centers coordinates (\emph{s}) to be scaled to the habitat grid (\emph{habitatGrid}) using the (\code{\link{scaleCoordsToHabitatGrid}} function.)
#'
...
#' dbinomLocal_normalBear(x=SparseY$y[i,,1],
...
#' dbinomLocal_normalBear(x=SparseY$yCombined[i,,1],
#'                    size=rep(1,4),
#'                    p0 = p0,
#'                    sigma= sigma, 
#'                    s=s[i,1:2],
