...
cat("model{
    ...
    lambda[j,1] <- exp(log.lambda.site[site[j]] + log.lambda.year[year_index[1]] + bYear.lam*year1[1] + bHQ1*hqCov1[j,1] + bHQ2*hqCov2[j,1] + bHQ3*hqCov3[j,1] + eps[j,1]) # year1 is t-1; year_index is t (to index properly the random effect)
    eps[j,1] ~ dnorm(0, tau)
    ...
    lambda[j,t] <- exp(log.lambda.site[site[j]] + log.lambda.year[year_index[t]] + bYear.lam*year1[t] + bHQ1*hqCov1[j,t] + bHQ2*hqCov2[j,t] + bHQ3*hqCov3[j,t] + eps[j,t])
    eps[j,t] ~ dnorm(0, tau)
    ...
    }",fill=TRUE, file = "3.HDS_trendmodel_lam[hqCAT]_sigHR_noRHO_butEPS.txt")
