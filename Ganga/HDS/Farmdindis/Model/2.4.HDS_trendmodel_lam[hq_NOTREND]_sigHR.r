...
cat("model{
    ...
    lambda[j,1] <- exp(log.lambda.site[site[j]] + log.lambda.year[year_index[1]] + bHQ*hqCov[j,1] + w[j,1]) #year_index is t (to index properly the random effect)
    ...
    }",fill=TRUE, file = "2.4.HDS_trendmodel_lam[hq_NOTREND]_sigHR.txt")
