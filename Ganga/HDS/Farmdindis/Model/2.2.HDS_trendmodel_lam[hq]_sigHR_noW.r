...
cat("model{
    ...
    lambda[j,1] <- exp(log.lambda.site[site[j]] + log.lambda.year[year_index[1]] + bYear.lam*year1[1] + bHQ*hqCov[j,1]) # year1 is t-1; year_index is t (to index properly the random effect)
    ...
    lambda[j,t] <- exp(log.lambda.site[site[j]] + log.lambda.year[year_index[t]] + bYear.lam*year1[t] + bHQ*hqCov[j,t])
    ...
    }",fill=TRUE, file = "2.2.HDS_trendmodel_lam[hq]_sigHR_noW.txt")
