...
# Round to assign to habitat category
hq_dat$WeightedQuality2011_CAT <- round(hq_dat$WeightedQuality2011)
hq_dat$WeightedQuality2021_CAT <- round(hq_dat$WeightedQuality2021)

which(hq_dat$WeightedQuality2011> 2.5)
which(hq_dat$WeightedQuality2021> 2.5)

# Really few transexts in hq 3 (transect AF24 from 2010-1015) -> Not going to work
# But I try
...
for (i in 1:nrow(hq_dat)) {
  hq[which(rownames(hq) %in% hq_dat$transectID[i]), 1:6] <- hq_dat$WeightedQuality2011_CAT[i]
  hq[which(rownames(hq) %in% hq_dat$transectID[i]), 7:13] <- hq_dat$WeightedQuality2021_CAT[i]
}
...
save(out, file = "3.Dat_HDS_trendmodel_lam[hq_weight_CAT1]_sigHR.RData")
...
setwd("D:/MargSalas/Ganga/Results/HDS/Farmdindis/Model_results")
load("3.Dat_HDS_trendmodel_lam[hq_weight_CAT1]_sigHR.RData")
...
text(x = mean_ab, y = 0.0007, labels = "Mean:\n73 ind", adj = 0, pos = 4, col = "darkslategrey", cex = 1, font = 2)
text(x = mode_ab, y = 0.0007, labels = "Mode:\n52 ind", adj = 0, pos = 4, col = "darkslategrey", cex = 1, font = 2)
