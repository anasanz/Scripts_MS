...
for (i in 1:nrow(hq_dat)) {
  hq[which(rownames(hq) %in% hq_dat$transectID[i]), 1:6] <- hq_dat$hq2011[i]
  hq[which(rownames(hq) %in% hq_dat$transectID[i]), 7:13] <- hq_dat$hq2012[i] # Its an error, it says 12 but its 2021
}
...
source("3.HDS_trendmodel_lam[hqCAT]_sigHR.r")

# With jagsUI 
out <- jags(data1, inits, params, "3.HDS_trendmodel_lam[hqCAT]_sigHR.txt", n.chain = nc,
            n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

setwd("D:/MargSalas/Ganga/Results/HDS/Farmdindis/Model_results")
save(out, file = "3.Dat_HDS_trendmodel_lam[hqCAT]_sigHR.RData")
...
setwd("D:/MargSalas/Ganga/Results/HDS/Farmdindis/Model_results")
load("3.Dat_HDS_trendmodel_lam[hqCAT]_sigHR.RData")
