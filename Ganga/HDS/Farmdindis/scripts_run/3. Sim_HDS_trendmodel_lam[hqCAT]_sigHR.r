...
# HQ COVARIATE (categorical)

bHQ1 <- 0.5
bHQ2 <- -0.6
bHQ3 <- 1

hq <- matrix(rbinom(max.sites*nyrs, 3, c(0.1, 0.3, 0.2, 0.4)), nrow = max.sites, ncol = nyrs)

# Create dummy variable

hq.dummy <- array(1, c(max.sites, nyrs, 3)) # 3th dimension includes 3 arrays: one per level (intercept doesnt count)
...
inits <- function(){list(mu.sig = runif(1, log(30), log(50)), sig.sig = runif(1), b = runif(1),
                         ...
                         bHQ1 = runif(1), bHQ2 = runif(1), bHQ3 = runif(1),
                         N = Nst)} 
...
save(out, file = "3.Sim_HDS_trendmodel_lam[hqCAT]_sigHR.RData")


setwd("D:/MargSalas/Ganga/Results/HDS/Farmdindis/Model_results")
load("3.Sim_HDS_trendmodel_lam[hqCAT]_sigHR.RData")
