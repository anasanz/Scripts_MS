...
# Abundance: random intercept in site and random effect in year, effect of habitat quality
# Detection: random intercept in observer, random year effect
...
# Sigma[jt] <- obs[jt] + year

# N[jt] ~ Pois(lambda[jt])
# log(lambda[jt]) <- site.alpha.ran[j] + year.ran[t] + beta*yr[t-1] + beta*hq[j,t] + W
