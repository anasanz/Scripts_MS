...
mu.b <- 0.997
sig.b <- 0.0438 # BUT this is not the sd, is the se!! sd = se x sqrt(N); Im missing N

# Intercept of sigma (Mean and SD of observer random effect)
#mu.sig <- 0.26297 # Mean in Valle del Ebro 
mu.sig <- 3.87+0.26 # Intercept + Mean in Valle del Ebro 
sig.sig <- 0.0690 # BUT this is not the sd, is the se!! sd = se x sqrt(N); Im missing N
