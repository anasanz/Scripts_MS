...
data1 <- list(nSites = nSites, mu.bHQ = mu.bHQ, sig.bHQ = sig.bHQ,
...
params <- c("Ntotal",
            "alpha", "mu.bHQ", "bHQ", "sigma")

## ---- MCMC settings ----
nc <- 3 ; ni <- 3000 ; nb <- 200 ; nt <- 2

## ---- Run model ----
setwd("D:/MargSalas/Scripts_MS/Ganga/HDS/SpecificPTS/Model")
source("2.HDS_sig_lam[hq[inf]].r")

# With jagsUI 
out <- jags(data1, inits, params, "2.HDS_sig_lam[hq[inf]].txt", n.chain = nc,
