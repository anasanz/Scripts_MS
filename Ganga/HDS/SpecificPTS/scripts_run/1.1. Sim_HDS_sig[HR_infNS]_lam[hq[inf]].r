...
# We will use an informative prior in sigma from the national PTS monitoring program
# Simulate mean and sd of sigma (it will be the prior)

mu.sig <- 3.55 
sig.sig <- 0.2941518 

sigma <- exp(rnorm(1, mu.sig, sig.sig))

# Beta
mu.b <- 0.997
sig.b <- 0.0438 

beta <- rnorm(1, mu.b, sig.b) # On the log scale
...
for(j in 1:nSites) {
  ...
  p <- g(x=d, sig=sigma, b = beta)   		# Detection probability. Sigma is site-time specific
  seen <- rbinom(N[j], 1, p)
  if(all(seen == 0))
    next
...
data1 <- list(nSites = nSites, mu.bHQ = mu.bHQ, sig.bHQ = sig.bHQ, mu.sig = mu.sig, sig.sig = sig.sig,
              mu.b = mu.b, sig.b = sig.b,
...
source("1.1.HDS_sig[HR_inf]_lam[hq[inf]].r")

# With jagsUI 
out <- jags(data1, inits, params, "1.1.HDS_sig[HR_inf]_lam[hq[inf]].txt", n.chain = nc,
