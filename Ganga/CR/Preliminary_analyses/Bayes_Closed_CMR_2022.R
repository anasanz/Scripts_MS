...
setwd("D:/MargSalas/Ganga/Results/Preliminary_2022")
#write.csv(out_dat_m0$summary, file = "out_dat_m0.csv")

# Summarize posteriors 
print(out_dat_m0, dig = 3) # p very low, 0.13
hist(out_dat_m0$sims.list$N, nclass = 50, col = "gray", main = "", xlab = "Population size", las = 1, xlim = c(35, 210))
...
hist(out_dat_mt$sims.list$N, nclass = 50, col = "gray", main = "", xlab = "Population size", las = 1, xlim = c(35, 200))
abline(v = nrow(data_ganga), lwd = 3)
...
# Call WinBUGS from R (BRT <1 min)
out_dat_mh <- jags(win.data2, inits, params, "model_mh.txt", n.chains = nc, 
                   n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
