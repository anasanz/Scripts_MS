...
## Different detection per group (2-part mixture model)
## ------------------------------------------------- 

# Same as sex but you don't provide any data for group belonging,
# so the model assigns the individuals of similar p to same group
...
data.fn <- function(N = 100, p = c(0.2,0.7), T = 3, psi = 0.5){
  ...
  group <- rbinom(n = N, size = 1, prob = psi) # 0 females, 1 males
  
  yfull <- yobs <- array(NA, dim = c(N, T))
  
  for (i in 1:N){
    for (j in 1:T){
      yfull[i,j] <- rbinom(n = 1, size = 1, prob = p[group[i]+1])
    }}
  ...
  return(list(N = N, p = p, C = C, T = T, yfull = yfull, yobs = yobs, group = group))
}
...
for (i in 1:M){
   ...
   for (j in 1:T){
      ...
      p.eff[i,j] <- z[i] * p[group[i]+1]		# Can only be detected if z=1
      } #j
   } #i
...
# Initial values (need to provide for all group variable)
inits <- function() list(z = rep(1, nrow(yaug)), p = runif(2, 0, 1),
                         group = c(rbinom(nrow(data$yobs) + nz,1,0.5)))
...
# Store N
 Ndf[i,] <- out_sim_M0_2partMixt$summary[rownames(out_sim_M0_2partMixt$summary) %in% "N", colnames(out_sim_M0_2partMixt$summary) %in% c("mean", "sd", "2.5%", "97.5%")]
 p1df[i,] <- out_sim_M0_2partMixt$summary[rownames(out_sim_M0_2partMixt$summary) %in% "p[1]", colnames(out_sim_M0_2partMixt$summary) %in% c("mean", "sd", "2.5%", "97.5%")]
 p2df[i,] <- out_sim_M0_2partMixt$summary[rownames(out_sim_M0_2partMixt$summary) %in% "p[2]", colnames(out_sim_M0_2partMixt$summary) %in% c("mean", "sd", "2.5%", "97.5%")]
}

setwd("D:/MargSalas/Ganga/Results/CR/Plots")
pdf("Summary_posteriors_100iter_p2part.pdf")

par(mfrow = c(3,1))
plot(density(Ndf$mean), main = "meanN_100iter", col = "darkcyan", lwd = 1.2, xlab = " ", ylab = " ", bty = "n", ylim = c(0,0.08), xlim = c(60,239))
...
plot(density(p1df$mean), main = "meaP1_100iter", col = "darkcyan", lwd = 1.2, xlab = " ", ylab = " ", bty = "n", ylim = c(0,25), xlim = c(0,1))
...
plot(density(p2df$mean), main = "meaP2_100iter", col = "darkcyan", lwd = 1.2, xlab = " ", ylab = " ", bty = "n", ylim = c(0,24), xlim = c(0,1))
