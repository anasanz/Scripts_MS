...
##          Cov in p0: Effort + Type of trap + month + month^2
## -------------------------------------------------
...
trap <- matrix(NA, nrow = max(Jyear), ncol = Tt)
for (t in 1:Tt){
  trap[1:Jyear[t],t] <- as.numeric(as.factor(as.matrix(tdf_list[[t]][1:Jyear[t], 4])))
}
trap[trap[] %in% c(2)] <- 0

#----   2.10. SAMPLING OCCASION COVARIATE (M) ---- 

m <- array(rep(rep(1:K, each = max(Jyear)), Tt), c(max(Jyear), K, Tt))
...
params<-c('N', 'gamma', 'sigma', 'p0', 'b.effort1', 'b.effort2', 'b.trap', 'b.month', 'b.month.sq',
          'phi', 'beta.dens', 'sigD','R', 'pc.gam', 'Nsuper')
...
chain_output <- parLapply(cl = this_cluster, X = 1:3, 
                          ...
                          code = SCRhab.Open.diftraps.3d.effortTrapSoccCov,   ##your model code
...
save(chain_output, file = "sampOpenSCR_diftraps_effortTrapSoccCov_1721_FINALDATA_3d.RData")
