...
edf <- edf[which(edf$session == 3), ] # Keep only detections of 2019 (year 3)
t <- edf %>% group_by(ind,occ, trap) %>%
  summarise(n()) %>%
  filter(ind == "Cannellito")
...
#setwd("~/Scripts_MS/Oso/PopDyn/SCR/Run_Data/Nimble/Results/1.SCRdenscov_year/Forest")
...
edf <- edf[which(edf$session == 3), ] # Keep only detections of 2019 (year 1)
