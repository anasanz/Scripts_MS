...
country <- raster("country.tif")
distcore <- raster("logDistcore_hrbear.tif")

# Crop both to extent of state-space
habitat.country <- crop(country, e) 
habitat.distcore <- crop(distcore, e) 


habitat.country.resampled <- resample(habitat.country, habitat.distcore, method = 'bilinear') # Resample so that both cov have the same extent
values(habitat.country.resampled)[values(habitat.country.resampled)>0.49] <- 1
values(habitat.country.resampled)[values(habitat.country.resampled)<0.5] <- 0
habitat.country.resampled[which(is.na(habitat.country.resampled[]))] <- 0 # First values are NA but belong to france, so set as 0
...
countryMask <- rasterize(Xbuf, habitat.country.resampled, mask = TRUE)

#which(is.na(distcoreMask[])) == which(is.na(countryMask[]))


## More NA in countrymask, need to solve
...
X.surv <- values(countryMask)[!is.na(countryMask[])]
...
save(nimData, nimConstants, 
     ...
     params, run_MCMC_allcode, modelcode, file = "Data_Model6-3.RData")
