...
roads6Mask <- rasterize(Xbuf, habitat.r, mask = TRUE)

#RETAIN HABITAT COORDINATES THAT ARE WITHIN THE HABITAT
G <- coordinates(roads6Mask)[!is.na(roads6Mask[]),]
colnames(G) <- c("x","y")
#PLOT CHECK 
plot(roads6Mask)
points(Xpoints)
...
X.d <- values(roads6Mask)[!is.na(roads6Mask[])]
...
modelcode = JS.SCRhab.Open.diftraps.sex.effortTrapBhCov.fast
...
save(nimData, nimConstants, 
     ...
     params, run_MCMC_allcode, modelcode, file = "Data_Model6-2.1.RData")
