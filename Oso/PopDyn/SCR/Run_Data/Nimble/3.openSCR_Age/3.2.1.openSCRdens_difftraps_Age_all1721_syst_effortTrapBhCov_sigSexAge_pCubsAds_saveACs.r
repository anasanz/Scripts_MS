...
source('3.9.SCRopen_diftraps_Age_trapsBhCov_sigsexAge_pCubsAds in Nimble.r')

##determine which parameters to monitor
params <- c("p.ad", "p.sub", "phi.ad","phi.sub","phi.cub", 
...
3000 + 1500 + 1500 + length(params)
# It would be saving 6021 cosas each iteration:
(150000 - 100000)/10 # niter-burning/thinning = 5000 iterations saved
5000*6021 # 30105000 -> 3*10^7 size of the vector
2000000000 # 2*10^9 (vector that r can save according to internet) but array? 
# Cyril says that he doesn't have problems, and he has way more data so no problem

###### SAVE FOR RUNNING #####

modelcode = SCRhab.Open.diftraps.age.effortTrapBhCov.sigsexage.pcubsads
...
save(nimData, nimConstants, 
     ...
     params, params2, run_MCMC_allcode, modelcode, file = "Data_Model3-3.2.RData")

# For Cyril
setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/SCR/Run_Data/Nimble/3.openSCR_Age/Data_server")
save(nimData, nimConstants, 
     inits, Tt, sex.in, piAGE.in, zstAGE, w.in, age.cat.in ,S.in.sc_coords, 
     params, params2, modelcode, file = "Data_Model3-3.2.1_CYRIL.RData")
...
model <- nimbleModel(SCRhab.Open.diftraps.age.effortTrapBhCov.sigsexage.pcubsads, constants = nimConstants, 
