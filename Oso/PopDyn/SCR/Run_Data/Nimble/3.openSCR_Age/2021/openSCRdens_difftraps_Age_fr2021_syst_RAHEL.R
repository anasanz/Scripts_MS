...
#setwd("~/Scripts_MS/Oso/PopDyn/SCR/Data/Systematic_prefinal_1721")
...
detNums[7,4]
detIndices[,,4]
sum(y.sp[7,,4][-16]) # Canellito, correcto
...
## CHECK ERROR CANELLITO:
# Try with the location of this trap where it was captured (more or less at the center of the grid,
# instead of average capture location)
...
model$logProb_y[,,4] # Año 2020, Canellito

# CHECK ERROR CANELLITO

plot(distcoreMask)
points(Xpoints[J.year[[4]], ])
caps <- which(Y[7,,4] > 0)
detIndices[7,,4]

points(Xt[[4]][caps,], col = "red")
points(as.numeric(apply(Xt[[4]][caps,],2,mean))) # Why the mean doesnt appear???
points(S.in[7,,4], col = "black")
points(Xt[[4]][288,], col = "black") # Try with this location instead of mean
