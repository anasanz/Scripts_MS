...
p0 <- -2.534601
b.effort <- 0.8 #effect of effort when measured per occasion
##effort per occasion is either 1 or 2, so intercept is set such that at effort =1
## baseline detection is 0.18; and at effort=2 it's 0.28
#plogis(p0+b.effort*1)
#plogis(p0+b.effort*2)
##this averages out at apprx 0.2 as it was set before

##if you want to simulate data without an effect of effort, you can set b.effort to 0
##and increase p0 to 0.549834; that will yield the same as the original simulation
...
n2<-round(prod(dim(effort))*0.3, dig=0) #ASP: To get how many numbers we will set as effort 2
...
for (t in 1:Tt){
  ...
  for (i in 1:n.all){
    if(z.all[i,t]==0){obs[i,,,t] <- 0} else{
      ...
      ##loop over occasions
      for (k in 1:K){
        #calculate effective baseline detection as function of effort
        p <- plogis(p0+b.effort*effort[1:nrow(Xt[[t]]),k,t]) # ASP: CHANGED! IT WAS effort[1:nrow(Xt[[t]]),,k])
        ...
        #combine the two components, resulting detection is individual, site
          ...
          p.eff<-p[j]*p.d[j] # Changed, it was p[j,k]
...
nimConstants <- list(
  ...
  nobs=n, Nyr=Tt, K=K , effort=effort
)
...
params<-c('N', 'gamma', 'sigma', 'p0', 'b.effort','phi', 'beta.dens', 'sigD','R', 'pc.gam', 'Nsuper')

#(1) set up model

model <- nimbleModel(SCRhab.Open.diftraps.3d.effort, constants = nimConstants, 
...
save(samp, file = "samp_difeff_3d.r")

# ELAPSED SYST TIME = 6048.34/3600 = 1.68 HORAS
...
### the following script reduces data back down to 2d and calculates total effort
### as a continuous site-level covariate, to test if that is adequate
### It also compares run time 3d vs 2d

##2d (per year) detection array; sum over occasions
y2d<-apply(y.in, c(1,2,4), sum, na.rm=TRUE)

##change to 'sparse' format - speeds up computation by reducing file size
y.sparse.2d <- getSparseY(y2d) 

## ASP: I have checked the function and there is no problem of having NA in the traps not sampled a given year

##extract pieces to be passed to Nimble - this is now as it was before
y.sp.2d <- y.sparse.2d$y
detIndices.2d <- y.sparse.2d$detIndices
detNums.2d <- y.sparse.2d$detNums
maxDetNums.2d <- y.sparse.2d$maxDetNums

#sum effort over occasions
effort.2d<-apply(effort, c(1,3), sum)
##scale so it ranges from 0 to 5 (0 then means effort was 1 on all occasions,
##that is what baseline detection then refers to)
# ASP: Because I changed the number of sampling occasions to 7, this would be 7
effort.2d<-effort.2d-7

##compile constants
nimConstants.2d <- list(
  M=Maug, J=Jyear, numHabWindows=numHabWindows, 
  numGridRows=numGridRows, numGridCols=numGridCols, 
  maxDetNums=maxDetNums.2d, MaxLocalTraps=MaxLocalTraps,
  nobs=n, Nyr=Tt, effort=effort.2d
  #whichtrap.year = whichtrap.year, Jyear.index = Jyear.index # To map the year specific trap arrays
)

##compile data
nimData.2d <- list(habDens=X.d, y=y.sp.2d,detNums=detNums.2d, 
                lowerHabCoords=lowerHabCoords, upperHabCoords=upperHabCoords,
                ...
                habitatGridDet=habitatGridDet,detIndices=detIndices.2d,
                detNums=detNums.2d, localTrapsIndex=localTrapsIndex, 
                localTrapsNum=localTrapsNum
)

##I believe inits should be the same, as parameters and data are identical

model.2d <- nimbleModel(SCRhab.Open.diftraps.effort, constants = nimConstants.2d, 
                     data=nimData.2d, inits=inits(), check = FALSE)
##ignore error message, only due to missing initial values at this stage

#(2) Compile model in c++
#     In complex models, this step can take a while (as well as step 5)
#     Much longer than in JAGS, but the model typically runs much faster
cmodel.2d <- compileNimble(model.2d)       

# (3) Configure MCMC - on an uncompiled model - this step allows setting which quantities to monitor
#     Also, nimble allows two sets of monitors, these can be thinned at different rates
#     all of which is more important in complex models but not to start with
conf.mcmc.2d<-configureMCMC(model.2d, monitors = params, thin=10)

# (4) Build the MCMC sampler based on configurations
mcmc.2d <- buildMCMC(conf.mcmc.2d)

# (5) Compile sampler in c++ together with compiled model
cmcmc.2d <- compileNimble(mcmc.2d, project = cmodel.2d, resetFunctions = TRUE)

# (6) Run (monitor time just for fun) [takes 20 seconds on my computer]
system.time(
  (samp.2d <- runMCMC(cmcmc.2d, niter = 5000, nburnin = 2000, nchains=3, inits = inits) )
)

setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/SCR/Run_Sim/Results/4.openSCRdenscov_Age_difeff")
save(samp.2d, file = "samp_difeff_2d.r")

# ELAPSED SYST TIME = 5656.25 /3600 = 1.57 HORAS
# 565625/6048.34 = 93.52; If 6048.34 is 100, the second model only ran 6.48 % faster

##remove NAs

inn<-colnames(samp.2d[[1]])
remm<-pmatch(c("R[1]", "pc.gam[1]"), inn)
samp2.2d<-lapply(samp.2d, function(x)x[,-remm])

##NOTE: summary command is from MCMCvis package; that also has good plotting options
## summary table for everything in "params" vector
summ.2d<-MCMCsummary(samp2.2d)
summ.2d
#MCMCtrace(samp)

setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/SCR/Run_Sim/Results/4.openSCRdenscov_Age_difeff")
saveRDS(list(summ, summ.2d), 'Results2v3d.rds')
