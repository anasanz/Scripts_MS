...
##            Effort reparametrized: 2p0 and 1b.effort
## -------------------------------------------------
...
### function to export p0 accordying to country within Nimble (as it doesn't tolerate a vector of indices)
## takes the two possible values for p0 and the country information and returns a vector of p0-values according to country
...
#p0, 2 different baseline detection probabilities for "france" and "spain"
p0 <- c(-2.534601, -4.50231)

# 1 beta for effort covariate if its 2 visits in France
...
country <- array(NA, c(n.max.traps, K, Tt)) 
...
effort.dummy <- array(NA, c(n.max.traps, K, Tt)) 
  ...
  for (i in 1:n.all){
    if(z.all[i,t]==0){obs[i,,,t] <- 0} else{
      ...
      for (k in 1:K){
        ...
        p0.country <- getp0(p0=p0, country=country[1:nrow(Xt[[t]]),k,t]) 
        
        #calculate effective baseline detection as function of effort (3 level)
        p <- plogis(p0.country+b.effort1*effort.dummy[1:nrow(Xt[[t]]),k,t]) 
...
nimConstants <- list(
  ...
  nobs=n, Nyr=Tt, K=K , effort=effort.dummy, country = country
)
...
source('4.1.1.SCRopen_diftraps_difeff_effort2p0Cov in Nimble.r')

##determine which parameters to monitor
params<-c('N', 'gamma', 'sigma', 'p0[1]','p0[2]', 'b.effort1', 'phi', 'beta.dens', 'sigD','R', 'pc.gam', 'Nsuper')
...
save(samp, file = "samp_difeff_3d_effor2p0Cov.r")
