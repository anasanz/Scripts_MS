...
#   # Covariate effects on p (b.effort1, b.effort2, b.effort3, b.trap)
trapBetas <- c(0.8,-0.4, 0.2, 0.3)
...
n4 <- round(prod(dim(effort))*0.1, dig=0)
effort[sample(1:prod(dim(effort)),n2, replace=FALSE)] <- 2
effort[sample(1:prod(dim(effort)),n3, replace=FALSE)] <- 3
effort[sample(1:prod(dim(effort)),n4, replace=FALSE)] <- 4


# Create dummy variable: HERE changes related to extra effort category 

# We need
# 000; 1 visit in France (cat 1): Intercept, no need to add
# 100; 2 visit in France (cat 2): Multiply b.effort1*array #1 
# 010; 1 visit Spain (cat 3): Multiply b.effort2*array #2
# 001; > 1 visit Spain (cat 4): Multiply b.effort3*array #3

effort.dummy <- array(1, c(n.max.traps, K, Tt, 3)) # 4th dimension includes 3 arrays: one per level (intercept doesnt count)

# effort.dummy[,,,1] =0  effort.dummy[,,,2] =0  effort.dummy[,,,3] =0 ==> 1 visit in France (cat 1): Intercept, no need to add
# effort.dummy[,,,1] =1  effort.dummy[,,,2] =0  effort.dummy[,,,3] =0 ==> 2 visit in France (cat 2): Multiply b.effort1*array #1 
# effort.dummy[,,,1] =0  effort.dummy[,,,2] =1  effort.dummy[,,,3] =0 ==> 1 visit Spain (cat 3): Multiply b.effort2*array #2
# effort.dummy[,,,1] =0  effort.dummy[,,,2] =0  effort.dummy[,,,3] =1 ==> >1 visit Spain (cat 3): Multiply b.effort3*array #3

for (t in 1:Tt){
  tmp <- tmp2 <- tmp3 <- tmp4 <- effort[,,t]
  
  # Dummy variable 2 visits in France (only de 2 appear as 1)
  tmp2[tmp2[] %in% c(1,3,4)] <- 0
  tmp2[tmp2[] %in% c(2)] <- 1
  effort.dummy[,,t,1] <- tmp2
  
  # Dummy variable 1 Visit in Spain (only de 3 appear as 1)
  tmp3[tmp3[] %in% c(1,2,4)] <- 0
  tmp3[tmp3[] %in% c(3)] <- 1
  effort.dummy[,,t,2] <- tmp3
  
  # Dummy variable >1 Visit in Spain (only de 4 appear as 1)
  tmp4[tmp4[] %in% c(1,2,3)] <- 0
  tmp4[tmp4[] %in% c(4)] <- 1
  effort.dummy[,,t,3] <- tmp4
}
...
effortarray <- array(1, c(n.max.traps, K, Tt, 4)) # HERE changes related to extra effort category (4 dim instead of 3)
effortarray[,,,1] <- effort.dummy[,,,1]
effortarray[,,,2] <- effort.dummy[,,,2]
effortarray[,,,3] <- effort.dummy[,,,3]
effortarray[,,,4] <- trap
...
## I need a matrix that intersects age category and sex to index sigma
sex_age <- matrix(rep(sex0, Tt, each = TRUE), nrow = n.all, ncol = Tt)
sex_age[ageTRUE == 1 | ageTRUE == 2] <- 0
sex_age[ageTRUE == 0] <- NA
# If it is unrecuited I think it doesn't matter, it doesn't play in the model
...
for (t in 1:Tt){
  ...
  for (i in 1:n.all){
    if(z.all[i,t]==0){obs[i,,,t] <- 0} else{
      ...
      p.d <- exp(-D[i,]^2/(2*sigma[sex_age[i,t]+1]^2)) ## 
      ...
      for (j in 1:nrow(Xt[[t]])){
        ...
        for (k in 1:K){
          if(k==1){
            ...
            p <- plogis(p0[ageTRUE[i,t]]+trapBetas[1]*effortarray[j,k,t,1] + trapBetas[2]*effortarray[j,k,t,2] + trapBetas[3]*effortarray[j,k,t,3] + trapBetas[4]*effortarray[j,k,t,4] + # HERE changes related to extra effort category (4 dim instead of 3)
                          b.bh*b[i,j,k,t])
...
inits<-function(){list(beta=c(0.15,rep(0.85/(Tt-1), Tt-1)), 
                       ...
                       p.sub=runif(1,0,0.5),
                       p.cub=runif(1,0,0.5),
                       trapBetas = runif(4, 0.5,1), # HERE changes related to extra effort category (4 dim instead of 3)
                       b.bh=runif(1, 0.5,1),
                       omega = runif(1, 0.5,1),
...
source('3.7.SCRopen_diftraps_Age_trapsBhCov_sigsexAge in Nimble.r') # HERE the model is the same, even with an extra effort voc
...
## Two errors, without nimble function:
#Error in getSymbolicParentNodesRecurse(code, constNames, indexNames, nimbleFunctionNames,  : 
#                                         R function 'ifelse' has arguments that cannot be evaluated; either the function must be a nimbleFunction or values for the following inputs must be specified as constants in the model: c("age.cat[i, 1]", "age.cat[i, 1]"),sex[i].
##ignore error message, only due to missing initial values at this stage

# With nimble function:
#[Note] Detected possible use of R functions in nimbleFunction run code.
#For this nimbleFunction to compile, 'ifelse' must be defined as a nimbleFunction, nimbleFunctionList, or nimbleList.
...
save(samp, file = "Results3-7.1.1.RData")

setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/SCR/Run_Sim/Results/4.OPSCR_Age")
load("Results3-7.1.1.RData")
summ7<-MCMCsummary(samp)
