...
##generate spatially correlated covariate for density
X.d <- spcov2(dg)

##generate 2 level categorical spatial covariate for survival
X.surv <- c(rep(0,(326/2)), rep(1,(326/2)))

##effect of covariate on density and survival
beta.d <- 1
beta.surv <- 0.5 # Survival in cells of 1 higher than in cells of 0 value
...
y.sp <- array(NA, c(M.aug, max.max, K, Tt))
...
params<-c('N', 'gamma', 'sigma', 'p0', 'trapBetas', 'b.bh', 'mu.phi', 'beta.dens', 'sigD','beta.cov',
          'Nsuper', 'omega')

#(1) set up model

model <- nimbleModel(JS.SCRhab.Open.diftraps.sex.effortTrapBhCov.fast, constants = nimConstants, 
...
save(samp, file = "Results6-1.0.4.RData")
