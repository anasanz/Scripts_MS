...
SCRhab.Open.diftraps.3d<-nimbleCode({
...
##################################################################################################################
SCRhab.Open.diftraps.3d.effort<-nimbleCode({
  ...
  for (t in 1:Nyr){
    ...
    for (k in 1:K){
      ...
      logit(p.eff[1:J[t],k,t])<-p0+b.effort*effort[1:J[t],k,t]
...
####################################################################################################################
SCRhab.Open.diftraps.effort<-nimbleCode({
  ...
  for (t in 1:Nyr){
    ...
    #calculate detection probability at all sites as function of overall effort
    logit(p.eff[1:J[t],t])<-p0+b.effort*effort[1:J[t],t]
    ...
    for(i in 1:M){
      ...
      y[i,1:maxDetNums,t]~dbinomLocal_normal(detNums = detNums[i,t],#getSparseY()$detNums
                                             ...
                                             p0Traps = p.eff[1:J[t],t], #model parameter
                                             sigma = sigma, #model parameter
                                             s = sxy[i,1:2,t], #model parameter
                                             trapCoords = X.sc[1:J[t],1:2,t], #trap coordinates (data); ASP: Year specific trap array
                                             localTrapsIndices = localTrapsIndex[1:numHabWindows,1:MaxLocalTraps[t],t], #from getLocalTraps()
