...
SCRhab.Open.diftraps.age.trapcov <- nimbleCode({
...
###############################################################################
  ...
  ## with covariate, this turns into an intercept on the logit scale
  ## so can be positive or negative, not bound between 0 and 1
  p.ad ~ dnorm(0,0.01)            # detection per age class
  p.sub ~ dnorm(0,0.01)  
  p.cub ~ dnorm(0,0.01)  
############################################################################### 
...
##################################################################################################
 ## covariate effects are always modeled on link scale, for a probability, it's logit scale
      logit(p0Traps[i,1:J[t],t]) <- p0[age.cat[i,t]+1] + bTrap * trapCov[1:J[t],t]
##################################################################################################
