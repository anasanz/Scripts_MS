...
SCRhab.Open.diftraps.age.effortTrapBhCov.sigsexage.DOsexage <- nimbleCode({
  ...
  beta.dens[1] ~ dnorm(0, 0.01)
  beta.dens[2] ~ dnorm(0, 0.01)
  ...
  mu1[1:numHabWindows,i] <- exp(beta.dens[i] * habDens[1:numHabWindows])
  sumHabIntensity[i] <- sum(mu1[1:numHabWindows,i])
  logHabIntensity[1:numHabWindows,i] <- log(mu1[1:numHabWindows,i])
  logSumHabIntensity[i] <- log(sumHabIntensity[i])
  }
...
#  ){
#    returnType(double(0))
#    sex.age.idx <- ifelse(((age.cat == 1) | (age.cat == 2)), 0, sex)
#    return(sex.age.idx)
#  })
