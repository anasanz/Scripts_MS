...
SCRhab.Open.diftraps.age.effortTrapBhCov.sigsexage.PR <- nimbleCode({
  
  ###recruitment probabilities
  for (t in 2:Nyr){
    ###pcr provided as data
    R[t]<-pcr* N.ad[t-1] #per capita recruitment times number adults in previous year=expected recruits
...
SCROpen.PR.distcore.t <- nimbleCode({
  ...
  ##effect of habitat cov on density
  for(t in 1:Nyr){
  beta.dens[t] ~ dnorm(0, 0.01)}
  
  ##model for density surface - vectorized, plus some other stuff for the function
  ##for initial AC 
  for(t in 1:Nyr){
  mu1[1:numHabWindows,t] <- exp(beta.dens[t] * habDens[1:numHabWindows])
  sumHabIntensity[t] <- sum(mu1[1:numHabWindows,t])
  logHabIntensity[1:numHabWindows, t] <- log(mu1[1:numHabWindows, t])
  ...
  for (i in 1:M){
    ...
    sxy[i, 1:2,1] ~ dbernppAC(
      lowerCoords = lowerHabCoords[1:numHabWindows, 1:2],#not used; from getWindowCoords()
      upperCoords = upperHabCoords[1:numHabWindows, 1:2],#not used; getWindowCoords()
      logIntensities = logHabIntensity[1:numHabWindows, 1], #from model
      logSumIntensity = logSumHabIntensity[1], #from model
      habitatGrid = habitatGrid[1:numGridRows,1:numGridCols],#from getWindowCoords()
      numGridRows =  numGridRows, #calculated from habitatGrid (data)
      numGridCols = numGridCols
    )
  ...
  for (t in 2:Nyr){
    ...
    for (i in 1:M){
      sxy[i, 1:2, t] ~ dbernppACmovement_normal(
        ...
        s                      = sxy[i, 1:2, t-1], #model parameter
        sd                     = sigD, #model parameter
        baseIntensities        = mu1[1:numHabWindows, t], 
        habitatGrid            = habitatGrid[1:numGridRows,1:numGridCols],
        numGridRows            = numGridRows,
        numGridCols            = numGridCols,
        numWindows             = numHabWindows
...
SCRhab.Open.diftraps.age.effortTrapBhCov.sigsexage.PR.pcr.fem <- nimbleCode({
...
SCROpen.PR.distcore.t.pcrFem <- nimbleCode({
