...
## OPTION 1 (doesnt work): two indexes
# 1. Index pasting which traps from X.sc are sampled each year
#whichtrap.year <- unlist(J.year) 

# 2. Index to map the whichtrap.year values from each year 
#Jyear.index <- NULL ## For Nimble: Index to map the number of traps trapID into years
#for (t in 1:Tt){
#  Jyear.index <- c(Jyear.index,rep(Yrs[[t]], Jyear[t]))}
#
#X.sc[whichtrap.year[Jyear.index == 1],] # Example, these are the traps from year 1


##OPTION 2 (doesnt work):
## function to join columns fillinG with NA
...
#} 
#whichtrap.year <- do.call(cbind.fill,J.year)
#X.sc[whichtrap.year[1:Jyear[1],1],] # Example, these are the traps from year 1
...
for(i in 1:M){
  ...
  if(ent.occ[i] <Tt){ 
    for(k in (ent.occ[i]+1):Tt){
      phi.it <- phi[ageTRUE[i, k-1]]
      zTRUE[i,k] <- rbinom(1, 1, zTRUE[i,k-1]*phi.it) # survival
      newage<-ageTRUE[i, k-1] +1
      ageTRUE[i, k] <- ifelse(newage>max.age, max.age, newage)             # aging
    }
  }
}
...
zstAGE <- zdatAGE
for(i in 1:n){
  if(last[i]<Tt) zstAGE[i,(last[i]+1):Tt] <- 0
}
...
system.time(
  (samp <- runMCMC(cmcmc, niter = 8000, nburnin = 4000, nchains=3, inits = inits) )
)
saveRDS(samp, 'TestOutputAGE.rds')
...
summ<-MCMCsummary(samp, Rhat = FALSE)
MCMCtrace(samp)
