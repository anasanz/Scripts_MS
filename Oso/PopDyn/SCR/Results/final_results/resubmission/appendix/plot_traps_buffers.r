...
Xbuf3 <- readOGR("Buffer_8500_traps_sxyObs_resub.shp")
...
pdf("SI_plot_traps_buffers_resub2.pdf",5,5)
...
plot(Xbuf, cex = 0.3, add = TRUE)
plot(Xbuf3, col = "darkgoldenrod", cex = 0.3, add = TRUE)
points(X, pch = 18, cex = 0.3)
points(m, pch = 18, col = "white", cex = 0.3)

dev.off()

# Exploration of which individuals fall outside:

Xbuf <- st_read("D:/MargSalas/Oso/Datos/GIS/Countries/Buffer_statespace.shp") %>%
  st_set_crs(st_crs(crs(sa)))
Xbuf2 <- st_read("D:/MargSalas/Oso/Datos/GIS/Countries/Buffer_8500_traps.shp") %>%
  st_set_crs(st_crs(crs(sa)))
Xbuf3 <- st_read("D:/MargSalas/Oso/Datos/GIS/Countries/Buffer_8500_traps_sxyObs_resub.shp") %>%
  st_set_crs(st_crs(crs(sa)))
m2 <- st_as_sf(m, coords = c("x","y")) %>%
  st_set_crs(st_crs(crs(sa)))
mapview(sa) + mapview(Xbuf) + mapview(Xbuf2) + mapview(Xbuf3) + mapview(m2, zcol = "red")

# Individual that fell in the border before but now outside: ID 39 (m = 14)
# Individuals that fall outside: 
#   m = 3,7,10,13,17: ID 13
#   m = 1,2,4,5,8,9,15,16: ID 3,5
# Individuals that fall in the border:
# m = 14: ID 39
# m = 11: ID 5
# m = 6, 12: ID 7

# Conclusion: 3 individuals outside, 2 in the border
