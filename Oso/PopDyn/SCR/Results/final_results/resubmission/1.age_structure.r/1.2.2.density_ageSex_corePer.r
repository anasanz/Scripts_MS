...
load("Densbuffer_newSize2.RData") # Obtained from script: 3.3. Core_vS_Periphery_polygonBigCore_ageCatSt.r
#### Density in the buffer with filled holes 

names(agesex_DENS_bear)
dim(agesex_DENS_bear)

colZn4 <- c("#9970ab", "#a6dba0")


setwd("D:/MargSalas/Oso/OPSCR_project/Results/Results_section/Resubmission/Plots")
pdf("1.2.2. Density_AgeSex_CorePer.pdf", 5, 5)

par(mfrow = c(4,2),
    mar = c(1,1,1.3,1),
    oma = c(2,3,2,2))

ageSt_list <- agesex_DENS_bear[-c(1)]
...
for (n in 1:nAgeSex){
  ...
  plot(1, ylim = c(0,scaleY_plots[n]), 
