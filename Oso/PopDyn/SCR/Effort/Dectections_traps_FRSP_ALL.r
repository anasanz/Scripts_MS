...
os1719_FR <- openxlsx::read.xlsx('Seguiment_Ossos_Pirineus_1996_2021_taula_final_2_cubLocations.xlsx') %>% 
  filter(Year %in% c(2017,2018,2019) & Confirmed_Individual != "Indetermined" & Country == "France") # Removed indetermined

os1719_FR <- os1719_FR %>% mutate(date = as_date(os1719_FR$Date_register, format = "%d/%m/%Y"),
                    month = month(date)) %>%
...
nrow(dat2_1719_FR[which(dat2_1719_FR$Obs_type == "Hair"),]) - nrow(dat2_1719_FR[which(dat2_1719_FR$Obs_type == "Hair" & dat2_1719_FR$Hair_trap == 1),]) # We remove 36 that are hair 
dat2_1719_FR[which(dat2_1719_FR$Obs_type == "Hair" & dat2_1719_FR$Hair_trap == 1),"method"] <- "piege poils" # Hair trap
...
setwd("D:/MargSalas/Oso/Datos/Tablas_finales/2022")
os1719_SP <- openxlsx::read.xlsx('Seguiment_Ossos_Pirineus_1996_2021_taula_final_2_cubLocations.xlsx') %>% 
  filter(Year %in% c(2017,2018,2019) & Confirmed_Individual != "Indetermined" & Country == "Spain") # Removed indetermined

os1719_SP <- os1719_SP %>% mutate(date = as_date(os1719_SP$Date_register, format = "%d/%m/%Y"),
                    month = month(date)) %>%
  filter(month < 12, month > 4) %>% # 7 months form may to november
  filter(!is.na(x_long)) # Remove observations without coordinates
...
setwd("D:/MargSalas/Oso/Datos/Tablas_finales/2022")
os2021_FR<- openxlsx::read.xlsx('Seguiment_Ossos_Pirineus_1996_2021_taula_final_2_cubLocations.xlsx') %>% 
  filter(Year %in% c(2020,2021) & Confirmed_Individual != "Indetermined" & Country == "France") # Removed indetermined

os2021_FR<- os2021_FR%>% mutate(date = as_date(os2021_FR$Date_register, format = "%d/%m/%Y"),
                    month = month(date)) %>%
  filter(month < 12, month > 4) %>% # 7 months form may to november
  filter(!is.na(x_long)) # Remove observations without coordinates
...
setwd("D:/MargSalas/Oso/Datos/Tablas_finales/2022")
os2021_SP <- openxlsx::read.xlsx('Seguiment_Ossos_Pirineus_1996_2021_taula_final_2_cubLocations.xlsx') %>%
  filter(Year %in%  c(2020,2021) & Confirmed_Individual != "Indetermined" & Country == "Spain") 
os2021_SP <- os2021_SP %>%
  mutate(date = as_date(os2021_SP$Date_register, format = "%d/%m/%Y"),
         month = month(date))

os2021_SP$month[302] <- 6 # Correct mannually because date was not exact and we only had month
os2021_SP$month[303] <- 4
os2021_SP$month[452] <- 9
os2021_SP$month[453] <- 9
...
#save(tdf2020, file = "tdf2020.RData")
#save(tdf2021, file = "tdf2021.RData")
...
# Check individual cases, to see if several observations are repeated in one plave
