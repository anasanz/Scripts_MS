...
```{r}
dat_cat <- readxl::read_xlsx("D:/MargSalas/Oso/Datos/Effort/France/Raw/Raw/Seguiment_2017_2019_Catalunya.xlsx") %>%
  janitor::clean_names() %>%
...
pts_foto_2017 <- dist_nearest(pts_foto_2017, trap_cat_foto) %>%
  st_drop_geometry() %>%
...
Navarre2018 <- rgdal::readOGR("D:/MargSalas/Oso/Datos/Effort/France/Raw/Raw/Donnees_Espagne_2017_2019/2018/Navarre2018.shp", use_iconv=TRUE, encoding = "UTF-8") %>%
  st_as_sf() %>%
  st_transform(crs = CRS("+proj=utm +zone=31 +datum=WGS84")) 
...
Aragon2018 <- rgdal::readOGR("D:/MargSalas/Oso/Datos/Effort/France/Raw/Raw/Donnees_Espagne_2017_2019/2018/Aragon2018.shp", use_iconv=TRUE, encoding = "UTF-8") %>%
  st_as_sf() %>%
  st_transform(crs = CRS("+proj=utm +zone=31 +datum=WGS84")) %>% 
  dplyr::select(FECHA, INDIVIDUO, TIPO.INDIC)

Aragon2019 <- rgdal::readOGR("D:/MargSalas/Oso/Datos/Effort/France/Raw/Raw/Donnees_Espagne_2017_2019/2019/Tabla OSO Aragn 2019.shp", use_iconv=TRUE, encoding = "UTF-8") %>%
  st_as_sf() %>%
  st_transform(crs = CRS("+proj=utm +zone=31 +datum=WGS84")) %>% 
  dplyr::select(FECHA, INDIVIDUO, TIPO.INDIC)
...
Constats_dommages_Total <- read.table("D:/MargSalas/Oso/Datos/Effort/France/Raw/Raw/Constats_dommages_Total.csv", dec=",", header=T, sep=";", fill=TRUE, encoding = "UTF-8") %>%
  janitor::clean_names() %>%
  rename(x = x_lamb_ii_etendu,
         y = y_lamb_ii_etendu,
         an = annee_indice,
...
Depred <- read_excel_allsheets("D:/MargSalas/Oso/Datos/Effort/France/Raw/Raw/Donnees_Espagne_2017_2019/Predation_Espagne.xlsx")
