...
#### 10.V3.1.CAM+HAIR Survival and detection specific to originally established age categories 
### INCLUDE ALSO CAMERA TRAP DATA TO DO THE DETECTION HISTORY!
...
HairStCH <- capt_hist_bear(data = os_id, 
                           method = c("Sampling_station","Transect"), 
                           obs_type = c("Hair", "Photo", "Video", "Photo/Video"))

chb <- as.data.frame(HairStCH$capt.hist)
rownames(chb) <- chb$Confirmed_Individual
chb <- as.matrix(chb[,c(5:16)]) 
chb <- chb[-which(apply(chb,1,sum)== 0), ]

n <- dim(chb)[1]
K <- dim(chb)[2]

nz <- n*3 # I augment the population three times the number of detected individuals
M <- n+nz
...
setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.1_CAM+HAIR")
saveRDS(list(out=out, summ=summ),"10.JS_V3.1_CAM+HAIR.rds")


## ---- Plot results ----

setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.1_CAM+HAIR")
out <- readRDS(file = "10.JS_V3.1_CAM+HAIR.rds")
out_sum <- out$summ
...
MCMCtrace(out$out,
          ...
          pdf = TRUE, 
          open_pdf = FALSE, 
          filename = '10.JS_V3.1_CAM+HAIR_Traceplot', 
          wd = 'D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.1_CAM+HAIR')
...
setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.1_CAM+HAIR")
pdf("10.JS_V3.1_CAM+HAIRParam.pdf", 7, 7)
