...
#### 10.V3.2. Survival and detection specific to different age categories structure 
## Cub = 1 
## Subadult = 2
## Adult = 3+ 
...
### age = 2: Subadult 1 (category 3) = YEARLING, 
### age >= 3: adult (category 4)
...
# here, this is highest age category excluding dead and unrecruited, so 3
max.age <- 3
...
## ASK RAHEL: THIS STEP IS NOT NEEDED IN THIS MODEL AS IT ONLY MATTERS THE AGE STRUCTURE AT YEAR 1 RIGHT?
...
table(ageMatAug[,1][which(ageMatAug[,1]>1)])/sum(table(ageMatAug[,1][which(ageMatAug[,1]>1)])) ## ASP: Those that have entered in the population
##no-one in cat 3 so create apprx. proportions (age cat 4)
piAGE.in<-c(0.2, 0.1, 0.7)
...
for (i in (n+1):M){
  if (ent.occ[i-n] == K){ # ASP: If the entered occasion is the last occasion (K)  
    ...
    #start in cat 3 on entry occasion, then continue series; set values >4 to 4
...
setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.3")
saveRDS(list(out=out, summ=summ),"10.JS_V3.3.rds")

## ---- Plot results ----

setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.3")
out <- readRDS(file = "10.JS_V3.3.rds")
out_sum <- out$summ
...
MCMCtrace(out$out,
          ...
          pdf = TRUE, 
          open_pdf = FALSE, 
          filename = '10.JS_V3.3_Traceplot', 
          wd = 'D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.3')
...
axis(2, c(1:length(params_p)), labels = c("Cub(1)", "Subad(2)", "Adult(>2)"), las = 2, cex.axis = 1)
...
axis(2, c(1:length(params_phi)), labels = c("Cub(1)", "Subad(2)", "Adult(>2)"), las = 2, cex.axis = 1)
...
axis(2, c(1:ncol(out$sims.list$piAGE)), labels = c("1y", "2y", ">3y"), las = 2, cex.axis = 1)
