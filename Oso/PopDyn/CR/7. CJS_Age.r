...
# Here individuals don't die...include deaths here or in latent state?
# --> better in latent state

#### Model 1: Phi[age_class] & p0 ####

# Bundle data
data_bear <- list(y = chb, f = f_bear, nind = dim(chb)[1], n.occasions = dim(chb)[2], z = known.state.cjs(chb), x = x_cat)
...
}",fill = TRUE, file = "cjs_phi[ageclass]_p0.txt")

# Initial values
inits <- function(){list(z = cjs.init.z(chb, f_bear), beta = runif(3, 0, 1), mean.p = runif(1, 0, 1))}  
...
# Call WinBUGS from R (BRT 3 min)
setwd("D:/MargSalas/Oso/SCR/Model")
outHairSt.ageclass <- jags(data_bear, inits, parameters, "cjs_phi[ageclass]_p0", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

print(outHairSt.ageclass, digits = 3)

# Summarize posteriors 
par(mfrow = c(1,1)) #Phi
hist(outHairSt.ageclass$sims.list$beta[,1], nclass = 50, col = adjustcolor("purple", alpha.f = 0.2), border =  adjustcolor("purple", alpha.f = 0.2), main = "", xlab = "Phi (Cub = purple; Subadult = green; Adult = orange)", las = 1, xlim = c(0.5,1))
abline(v = mean(outHairSt.ageclass$sims.list$beta[,1]), col = "purple", lwd = 3)

hist(outHairSt.ageclass$sims.list$beta[,2], nclass = 50, col = adjustcolor("green", alpha.f = 0.2), border = adjustcolor("green", alpha.f = 0.2),  add = TRUE)
abline(v = mean(outHairSt.ageclass$sims.list$beta[,2]), col = "darkgreen", lwd = 3)

hist(outHairSt.ageclass$sims.list$beta[,3], nclass = 50, col = adjustcolor("orange", alpha.f = 0.2), border = adjustcolor("orange", alpha.f = 0.2),  add = TRUE)
abline(v = mean(outHairSt.ageclass$sims.list$beta[,3]), col = "darkorange", lwd = 3)

par(mfrow = c(1,1)) # P detection
hist(outHairSt.ageclass$sims.list$p.g[,1], nclass = 50, col = adjustcolor("grey", alpha.f = 0.2), border =  adjustcolor("purple", alpha.f = 0.2), main = "", xlab = "P (F = purple; M = green)", las = 1, xlim = c(0.5,1))
abline(v = mean(outHairSt.ageclass$sims.list$mean.p), col = "grey", lwd = 3)


# Save outputs
setwd("D:/MargSalas/Oso/SCR/Exp_analysis")
write.csv(outHairSt.ageclass$summary, file = "outHairSt_phi[ageclass]_p0.csv")
...
# Initial values
inits <- function(){list(z = cjs.init.z(chb, f_bear), beta.phi = runif(3, 0, 1), beta.p = runif(3, 0, 1))}  
...
# Save outputs
setwd("D:/MargSalas/Oso/SCR/Exp_analysis")
write.csv(outHairSt.ageclass$summary, file = "outHairSt_phi[ageclass]_p[ageclass].csv")
