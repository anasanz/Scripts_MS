...
}",fill = TRUE, file = "cjs-cov-raneff.txt")
...
cjs.cov <- jags(bugs.data, inits, parameters, "cjs-cov-raneff.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)
...
dist_path <- dist_path$mean
dist_path_mean <- mean(dist_path)
dist_path_sd <- sd(dist_path)
dist_path_sc <- (dist_path - dist_path_mean) / dist_path_sd
...
# JAGS data
data_bear <- list(y = chb, f = f_bear, nind = dim(chb)[1], n.occasions = dim(chb)[2], z = known.state.cjs(chb), x = dist_path_sc)
...
outHairSt_distPath <- jags(data_bear, inits, parameters, "cjs-cov-raneff.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

print(outHairSt_distPath, digits = 3)

# Summarize posteriors 

hist(outHairSt_distPath$sims.list$phi, nclass = 50, col = "gray", main = "", xlab = "Phi", las = 1, xlim = c(0,1))
abline(v = mean(outHairSt_distPath$sims.list$phi), col = "blue", lwd = 3)

hist(outHairSt_distPath$sims.list$beta, nclass = 50, col = "gray", main = "", xlab = "Dist_path", las = 1, xlim = c(-10,2))
abline(v = mean(outHairSt_distPath$sims.list$beta), col = "blue", lwd = 3)

# Save outputs
setwd("D:/MargSalas/Oso/SCR/Exp_analysis")
write.csv(HairStCH$capt.hist, file = "HairStCH_Cat_DistPath.csv")
write.csv(outHairSt_distPath$summary, file = "outHairSt_Cat_DistPath.csv")
