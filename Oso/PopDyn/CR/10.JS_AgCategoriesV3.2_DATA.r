...
### age = 2: Subadult 1 (category 3), 
### age = 3: subadult 2 (category 4)
### age >=4: adult (category 5)
...
# here, this is highest age category excluding dead and unrecruited, so 4
max.age <- 4
...
for (i in (n+1):M){
  if (ent.occ[i-n] == K){ # ASP: If the entered occasion is the last occasion (K)  
    ...
    #start in cat 3 on entry occasion, then continue series; set values >5 to 5
...
setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.2")
saveRDS(list(out=out, summ=summ),"10.JS_V3.2.rds")

## ---- Plot results ----

setwd("D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.2")
out <- readRDS(file = "10.JS_V3.2.rds")
out_sum <- out$summ
...
MCMCtrace(out$out,
          ...
          pdf = TRUE, 
          open_pdf = FALSE, 
          filename = '10.JS_V3.2_Traceplot', 
          wd = 'D:/MargSalas/Scripts_MS/Oso/PopDyn/CR/Results/10.JS_V3.2')
...
axis(2, c(1:length(params_p)), labels = c("Cub(1)", "Subad(2+3)", "Adult(>3)"), las = 2, cex.axis = 1)
...
axis(2, c(1:length(params_phi)), labels = c("Cub(1)", "Subad(2+3)", "Adult(>3)"), las = 2, cex.axis = 1)
...
axis(2, c(1:ncol(out$sims.list$piAGE)), labels = c("1y", "2y", "3y", ">4y"), las = 2, cex.axis = 1)
