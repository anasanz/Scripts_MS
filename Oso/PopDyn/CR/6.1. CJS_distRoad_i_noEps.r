...
winter <- rnorm(sum(marked), 0, 1^0.5)
logit.phi <- qlogis(mean.phi) + beta*matrix(winter,nrow = sum(marked), ncol = n.occasions-1)
#+ eps
...
}",fill = TRUE, file = "cjs-cov-raneff[i]_noEps.txt")
...
setwd("D:/MargSalas/Oso/SCR/Model")
cjs.cov.i.noEps <- jags(bugs.data, inits, parameters, "cjs-cov-raneff[i]_noEps.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

# Summarize posteriors+
print(cjs.cov.i.noEps, digits = 3)

# Produce graph
par(mfrow = c(1, 2), las = 1)
hist(cjs.cov.i.noEps$sims.list$beta, nclass = 25, col = "gray", main = "", xlab = expression(beta), ylab = "Frequency")
abline(v = -0.3, col = "red", lwd = 2)
hist(cjs.cov.i.noEps$sims.list$sigma2, nclass = 50, col = "gray", main = "", xlab = expression(sigma^2), ylab = "Frequency", xlim=c(0, 3))
...
# --- Mean distance to roads per individual ----
...
# Co variate distance to road
# One value per individual: Individuals stay at an average distance to roads all their life

# Standardize
dist_mean <- mean(dist_sum$mean)
dist_sd <- sd(dist_sum$mean)
dist_sc <- (dist_sum$mean - dist_mean) / dist_sd
...
inits <- function(){list(z = cjs.init.z(chb, f_bear), mu = rnorm(1), beta = runif(1, -5, 5), mean.p = runif(1, 0, 1))}  
...
setwd("D:/MargSalas/Oso/SCR/Model")
outHairSt_distRoad_i_noEps<- jags(data_bear, inits, parameters, "cjs-cov-raneff[i]_noEps.txt", n.chains = nc, n.thin = nt, n.iter = ni, n.burnin = nb, parallel = TRUE)

print(outHairSt_distRoad_i_noEps, digits = 3)

# Summarize posteriors 
par(mfrow = c(1,1))
hist(outHairSt_distRoad_i_noEps$sims.list$mean.phi, nclass = 50, col = "gray", main = "", xlab = "Phi", las = 1, xlim = c(0.5,1))
abline(v = mean(outHairSt_distRoad_i_noEps$sims.list$mean.phi), col = "blue", lwd = 3)

hist(outHairSt_distRoad_i_noEps$sims.list$beta, nclass = 50, col = "gray", main = "", xlab = "Dist_road", las = 1, xlim = c(-10,2))
abline(v = mean(outHairSt_distRoad_i_noEps$sims.list$beta), col = "blue", lwd = 3)

# Save outputs
setwd("D:/MargSalas/Oso/SCR/Exp_analysis")
write.csv(outHairSt_distRoad_i_noEps$summary, file = "outHairSt_Cat_DistRoad[i]_noEps.csv")
