...
setwd("D:/MargSalas/Oso/Datos/Tablas_finales/2023")
os <- openxlsx::read.xlsx('Seguiment_Ossos_Pirineus_1996_2023_TAULA_FINAL.xlsx')


# Load table info individuals to fill out sex and ages of cubs
setwd("D:/MargSalas/Oso/Datos/Tablas_finales/2023")
info <- readxl::read_xlsx("D:/MargSalas/Oso/Datos/Tablas_finales/2023/Taula_General_Dades_DEFINITIVA(SFF).xlsx", sheet = 1)
info <- info[,c(6,5,42)]
colnames(info)[1] <- "Probable_Individual"
colnames(info)[2] <- "Sex"
colnames(info)[3] <- "Year_birth"
...
for (i in 1:nrow(os)){
   ...
   if (os$N_cubs_observed[i] == os$N_cubs_estimated[i] & !is.na(os$N_cubs_estimated[i]) & !is.na(os$N_cubs_observed[i])) {
     ...
     for (j in (1:as.numeric(os$N_cubs_observed[i]))) { # Add as many rows as cubs
       ...
       add_row$Date_register_formated <- as.Date(add_row$Date, format = "%d/%m/%Y") # For Age class
...
# How many mother detections have been traslated into cub detections? 562!
...
for (i in 1:nrow(os)){
  ...
  if (os$Remarks[i] == "The cub is Noisette" & !is.na(os$Remarks[i])) {
    add_row <- os[i,]     # Row to add with the different names of the cubs
    #cubs <- "Noisette" # Cubs to add (F025 in new encoding)
    cubs <- "F025" # Cubs to add (F025 in new encoding)
...
# There are only 33 mother observations where we can't identify the cub, so we leave it as it is.
...
setwd("D:/MargSalas/Oso/Datos/Tablas_finales/2023")
openxlsx::write.xlsx(os, 'Seguiment_Ossos_Pirineus_1996_2023_taula_final_cubLocations.xlsx')
