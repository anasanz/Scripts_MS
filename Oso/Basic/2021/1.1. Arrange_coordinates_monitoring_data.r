...
os$Age2 <- ifelse(is.na(os$Age2), os$Age2 <- os$Age, os$Age2)
os <- os[,-c(4,31)]
os <- os[,c(1:3,30,4:29)]
colnames(os)[4] <- "Age"
...
# Sort out columns
os_data <- os_data[,c(1:12,31,32,13:30)]

# Remove duplicates 

os_data <- os_data[-which(duplicated(os_data)), ]
