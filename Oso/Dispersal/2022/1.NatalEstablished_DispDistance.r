...
# Warning messages on "CRS object has comment" and "xy should contain only one column" are no problem
...
for(i in 1:length(id)){
  ...
  #pos_est <- pos_est[-which(is.na(pos_est$Year)), ] # Esto son errores y pasan raramente (en el caso de Boutxy en radiot), hay que comprobar pero de momento elimino
...
saveRDS(mcp_natal, "mcp_natal.rds")
saveRDS(mcp_est1, "mcp_est1.rds")
saveRDS(mcp_est2, "mcp_est2.rds")
