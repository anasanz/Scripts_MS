...
for (i in 1:length(id_males)) {
  ...
  setwd("D:/MargSalas/Oso/Datos/Plots/2022/Detailed_ID2/males")
...
for (i in 1:length(id_females)) {
  os_id <- os_females[which(os_females$Confirmed_Individual_cub == id_females[i]), ] # Select individual
  df <- os_id@data
  ext <- data.frame(bbox(os_id)) # Extent
  myMap <- get_map(location = bbox(os_id), source = "stamen", maptype = "terrain", crop = FALSE) # Basemap
  
  mcp_nat_id <- which(names_mcp_nat %in% id_females[i]) # MCP natal
  
  ifelse(length(mcp_nat_id) > 0, x <- mcp_natal[[which(names_mcp_nat %in% id_females[i])]], x <- 0)
  ...
  if (length(x) >= 1){ # If there is a mcp polygon
    ...
    if(id_females[i] %in% d$ID) { # And there is enough positions to create a natal-est arrow
      ...
      if(!is.na(d[d$ID %in% id_females[i],c(4)])) {
        ...
        p2 <- ggmap(myMap)+
          ...
          geom_polygon(data = x[[2]], aes(x = long, y = lat, group = group), 
                       colour = "black", fill = "transparent", size = 0.5) +
          geom_segment(aes(x = d[d$ID %in% id_females[i],c(2)], y = d[d$ID %in% id_females[i],c(3)], 
                           xend = d[d$ID %in% id_females[i],c(4)], yend = d[d$ID %in% id_females[i],c(5)]),
                       arrow = arrow(length = unit(0.1, "cm")),
                       lwd = 0.2) +
          ggtitle("Average location")
        ...
        geom_polygon(data = x[[2]], aes(x = long, y = lat, group = group), 
                     colour = "black", fill = "transparent", size = 0.5) +
        geom_segment(aes(x = d[d$ID %in% id_females[i],c(2)], y = d[d$ID %in% id_females[i],c(3)], 
                         xend = d[d$ID %in% id_females[i],c(6)], yend = d[d$ID %in% id_females[i],c(7)]),
                     arrow = arrow(length = unit(0.1, "cm")),
                     lwd = 0.2) +
        ggtitle("Average location") }} else {p2 <- ggmap(myMap)+
          geom_point(data = mean_loc, aes(x = meanx , y = meany, colour = factor(Year), shape = factor(Age_class_cub)), 
  ...
  # Common plot:
  # TITLE
  title1 = text_grob(paste(id_females[i]," (",info$Year_birth[info$ID %in% id_females[i]], "-", info$Year_death[info$ID %in% id_females[i]], ")", sep = ""), size = 15, face = "bold")
  ...
  setwd("D:/MargSalas/Oso/Datos/Plots/2022/Detailed_ID2/females")
  pdf(paste(id_females[i], ".pdf", sep = ""),
      width = 8, height = 5)
  grid.arrange(p0,p1,p2, nrow = 2, ncol = 2,
               top = title1,
               shared_legend)
